...
# Settign custom Page Title and Icon
st.set_page_config(page_title='Mask Detector', page_icon='😷', layout='centered', initial_sidebar_state='expanded')
...
def mask_image():
    ...
    # Pass the blob throgh the network and obtain the face detections
    ...
    for i in range(0, detections.shape[2]):
        # extract the configdence (i.e., probability) associated with the detection 
        ...
        if confidence > 0.5:
            ...
            # inlude the probability in the label 
            label = "{}: {:.2f}%".format(label, max(mask, withoutMask) * 100)

            # display the label and bounding box rectable on teh output frame
