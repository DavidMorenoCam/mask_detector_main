...
ap.add_argument("-d", "--query", required=True, help="search query to search Bing Image API for")
ap.add_argument("-o", "--output", required=True, help="path to output directory of iamges")
...
params = {"q": terms, "offset": 0, "count": GOUP_SIZE}
print("[INFO] searching Bing API for '{}'".format(term))
search = requests.fet(URL, headers=headers, params=params)
...
for offset in range(0, estNumResults, GROUP_SIZE):
    ...
    for v in results["value"]:
        try:
            ...
            if type(e) in EXECTIONS: 
                print("[INFO] skupping: {}".format(v["contentUrl"]))
                continue
        image = cv2.imread(p)
        if image is None:
